    ...
    <div class="container">
      <div class="row">
        <div class="col-md-9 col-md-push-3">
          ...
          <?php
          $sql = "SELECT tblvehicles.*,tblbrands.BrandName,tbllocation.LocationName,tblbrands.id as bid 
          FROM tblvehicles 
          ...
          if ($query->rowCount() > 0) {
            foreach ($results as $result) {
              $vid = $result->id;
              $status = "SELECT tblbooking.FromDate, tblbooking.ToDate, tblbooking.Status FROM tblbooking WHERE VehicleId = :vid";
              $squery = $dbh->prepare($status);
              $squery->bindParam(':vid', $vid, PDO::PARAM_INT);
              $squery->execute();
              $sresults = $squery->fetchAll(PDO::FETCH_OBJ);
              $today = date('Y-m-d');
              $available = true; // Assume the vehicle is available by default
              // Check if there are any bookings for this vehicle
              if ($squery->rowCount() > 0) {
                foreach ($sresults as $sresult) {
                  // Check booking status
                  if ($sresult->Status == 1) {
                    // Check if today's date falls within any booking date range
                    if ($today >= $sresult->FromDate && $today <= $sresult->ToDate) {
                      $available = false; // Vehicle is not available
                      break; // No need to continue checking bookings
                    }
                  }
                }
              }

              if ($available) { // Display vehicle only if available
          ?>
