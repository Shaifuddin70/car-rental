								...
								<div class="col-md-12">
									<div class="panel panel-default">
										...
										<div class="panel-body">
											...
											<?php
											$id = intval($_GET['id']);
											$sql = "SELECT tblvehicles.*,tblowner.UserName,tblowner.id as oid, tblbrands.BrandName, tblbrands.id as bid, tbllocation.LocationName, tbllocation.id as lid
											FROM tblvehicles
											JOIN tblbrands ON tblbrands.id = tblvehicles.VehiclesBrand
											JOIN tblowner ON tblowner.id = tblvehicles.owner_id
											JOIN tbllocation ON tbllocation.id = tblvehicles.location
														...
														<div class="form-group">
															...
															<div class="col-sm-4">
																<select class="selectpicker" name="ownername" required>
																	<option value="<?php echo htmlentities($result->oid); ?>"><?php echo htmlentities($bdname = $result->UserName); ?></option>
																	<?php $ret = "select id,UserName from tblowner";
																	$query = $dbh->prepare($ret);
																	//$query->bindParam(':id',$id, PDO::PARAM_STR);
																	$query->execute();
																	$oresults = $query->fetchAll(PDO::FETCH_OBJ);
																	if ($query->rowCount() > 0) {
																		foreach ($oresults as $oresult) {
																	?>
																			<option value="<?php echo htmlentities($oresult->id); ?>"><?php echo htmlentities($oresult->UserName); ?></option>
																	<?php }
																	} ?>
														...
														<div class="form-group">
															...
															<div class="col-sm-4">
																<select class="selectpicker" name="location" required>
																	<option value="<?php echo htmlentities($result->lid); ?>"><?php echo htmlentities($bdname = $result->LocationName); ?></option>
